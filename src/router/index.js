...
const dynamicRoutes = [
	{
		...
		// redirect: '/home', // 可通过修改转发达到默认显示的目的
		meta: {
			isKeepAlive: true,
		},
		children: [],
...
export async function adminUser(router) {
	resetRouter();
	let res = Session.get('routers');
	await store.dispatch('routesList/setRoutesList', res);
	getFirstPage(res);
	dynamicRoutes[0].children = res;
	const awaitRoute = dynamicRouter(dynamicRoutes);

	// [...awaitRoute, { path: '*', redirect: '/404' }].forEach((route) => {
	[...awaitRoute].forEach((route) => {
		router.addRoute({ ...route });
	});
	setCacheTagsViewRoutes(JSON.parse(JSON.stringify(res)));
...
function getFirstPage(res) {
	if (Array.isArray(res) && res.length > 0) {
		...
		if (item.children) {
			...
			dynamicRoutes[0].redirect = item.path;
		}
	}
}
...
router.beforeEach((to, from, next) => {
	...
	if (['/loginCallbackByFeishu', '/loginCallbackByQiwei', '/loginCallbackByDingding'].includes(to.path)) {
		next();
